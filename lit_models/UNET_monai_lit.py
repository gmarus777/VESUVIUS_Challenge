...
class UNET_lit(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        fbeta_score_1 = FBetaScore(task="binary", beta=.5, threshold=.1, ignore_index=0).to(DEVICE)
        fbeta_score_4 = FBetaScore(task="binary", beta=.5, threshold=.4, ignore_index=0).to(DEVICE)
        fbeta_score_6 = FBetaScore(task="binary", beta=.5, threshold=.6, ignore_index=0).to(DEVICE)
        fbeta_score_75 = FBetaScore(task="binary", beta=.5, threshold=.75, ignore_index=0).to(DEVICE)
        fbeta_score_83 = FBetaScore(task="binary", beta=.5, threshold=.83, ignore_index=0).to(DEVICE)
        fbeta_score_90 = FBetaScore(task="binary", beta=.5, threshold=.9, ignore_index=0).to(DEVICE)
        fbeta_score_95 = FBetaScore(task="binary", beta=.5, threshold=.95, ignore_index=0).to(DEVICE)
