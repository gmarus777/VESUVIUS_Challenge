...
class Lit_Model(pl.LightningModule):
    ...
    def _init_loss(self, y_pred, y_true):
        ...
        return 0.5*self.loss_bce(y_pred , y_true.float())  + 0.1*self.loss_tversky(y_pred , y_true.float())
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.cfg.learning_rate, weight_decay=self.cfg.weight_decay)
