...
class Lit_Model(pl.LightningModule):
    def __init__(
        ...
        self.loss_bce = smp.losses.SoftBCEWithLogitsLoss(pos_weight=torch.tensor(0.75))  # pos_weight=torch.tensor(1)
    ...
    def _init_loss(self, y_pred, y_true):
        ...
        #return self.loss_bce(y_pred , y_true.float())  + self.loss_tversky(y_pred , y_true.float()) #+ 0.5*self.loss_lovasz(y_pred , y_true.float())
        return  self.loss_bce(y_pred , y_true.float())  + self.loss_dice(y_pred , y_true.float())
    ...
    def training_step(self, batch, batch_idx):
        ...
        images, labels, binary_masks = batch
        labels = labels.long()
