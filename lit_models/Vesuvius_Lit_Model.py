...
class Lit_Model(pl.LightningModule):
    def __init__(
        ...
        self.loss_bce = smp.losses.SoftBCEWithLogitsLoss(pos_weight=torch.tensor(0.1))  # pos_weight=torch.tensor(1)
    ...
    def _init_loss(self, y_pred, y_true):
        ...
        return self.loss_bce(y_pred , y_true.float())  +  0.5*self.loss_tversky(y_pred , y_true.float()) #+ 0.4*self.loss_focal(y_pred , y_true.float())
