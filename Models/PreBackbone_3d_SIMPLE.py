...
class PreBackbone_3D_SIMPLE(nn.Module):
    def __init__(self, z_dim=48, emdedding_dims=4, conv_dimensions=[16, 32, 64], filter_sizes=[16, 32, 64],
                 batch_norm=False):
    ...
    def forward(self, x):
        ...
        x_orig, x_att = self.embed_layer(
            x)  # (B, 1, Z, H, W) -> (B, emdedding_dims[0], Z/2, H, W) and (B, 1, Z/2, H/4, W/4)
